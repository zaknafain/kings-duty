...
export class MapService {
  private readonly _map = new BehaviorSubject<Tile[]>([]);
  ...
  set map(tiles: Tile[]) {
    this._map.next(tiles);
  }

  revealTile(tile: Tile): void {
    this.map.find(t => t.x === tile.x && t.y === tile.y).isKnown = true;

    if (this.map.findIndex(t => t.y > tile.y) < 0) {
    ...
    if (this.map.findIndex(t => t.x > tile.x) < 0) {
  ...
  private addTile(x: number, y: number): void {
    ...
    if (this.map.findIndex(tile => tile.y === y) < 0) {
      const rowZero = this.map.filter(tile => tile.y === 0);
      ...
      const columnZero = this.map.filter(tile => tile.x === 0);
    ...
    const newMap = [...this.map, ...newTiles];

    this.map = newMap.sort((a: Tile, b: Tile) => this.sortTiles(a, b));
  }
